...
const MODULE_PATH: &[&str] = &["signal_cloud", "capability"];

fn main() {
    let crate_root = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("manifest dir set"));
    let source_path = crate_root.join("schema/capability.ethos");
    let rust_path = crate_root.join("src/schema/capability/generated.rs");

    println!("cargo:rerun-if-changed=schema/capability.ethos");
    println!("cargo:rerun-if-changed=src/bootstrap_manifest.rs");
    println!("cargo:rerun-if-changed=src/schema/capability/generated.rs");

    let source = fs::read_to_string(&source_path).expect("read capability Interface source");
    ...
    .expect("assemble authority-approved capability Interface transaction");
    let rust = rust_logos();
    let type_paths = CapabilityRustTypePaths::new();
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("capability bootstrap catalog is complete")
}
...
struct CapabilityRustTypePaths(BTreeMap<VocabularyEncodedId, RustTypePath>);

impl CapabilityRustTypePaths {
    fn new() -> Self {
        Self(BTreeMap::from([
            (
...
impl RustTypePathResolver for CapabilityRustTypePaths {
